	...
	public Iterator<StoredMessage> getAllArrivedMessages(
		...
		return new Iterator<StoredMessage>() {
			...
			public StoredMessage next() {
				...
				try {
					messageId = c.getString(c.getColumnIndexOrThrow(MqttServiceConstants.MESSAGE_ID));
					clientHandle = c.getString(c.getColumnIndexOrThrow(MqttServiceConstants.CLIENT_HANDLE));
					topic = c.getString(c.getColumnIndexOrThrow(MqttServiceConstants.DESTINATION_NAME));
